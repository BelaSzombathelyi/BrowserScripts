// ==UserScript==
// @name         Garmin Connect → Markdown (Firefox/mobil, v3.1.0, szerver nélkül)
...
(function () {
    ...
    function injectStyle() {
        ...
        style.textContent = `
            ...
            #${BTN_ID}.gc-save { background: #16a34a; }
            #${BTN_ID}.gc-save:active { background: #15803d; }
            #${STATUS_ID} {
                flex: 1; min-width: 120px; font-size: 12px; color: #cbd5e1;
                white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
            }
            #${CLOSE_ID} {
                border: none; background: transparent; color: #94a3b8; font-size: 22px;
    ...
    // A kész MD ideiglenes tárolása a kétlépcsős (Firefox-barát) letöltéshez.
    let preparedDownload = null; // { filename, content }

    // Az overlay tényleges magasságához igazítja a body felső paddingjét (mobilon több sorba tördelődhet).
    function syncBodyPadding() {
        const overlay = document.getElementById(OVERLAY_ID);
        document.body.style.paddingTop = overlay ? `${overlay.offsetHeight}px` : '';
    }

    // A fő gomb két állapota: 'prepare' (beolvasás) és 'save' (kész MD mentése).
    function setButtonMode(btn, mode) {
        btn.dataset.mode = mode;
        if (mode === 'save') {
            btn.textContent = '⬇️ Mentés (koppints)';
            btn.classList.add('gc-save');
        } else {
            btn.textContent = '📥 Markdown letöltése';
            btn.classList.remove('gc-save');
        }
        syncBodyPadding();
    }

    // 2. lépés: friss user-activationön belül fut (koppintás), így a Firefox AZONNAL letölt – nem kell egeret mozgatni.
    function doSave(btn) {
        if (!preparedDownload) { setButtonMode(btn, 'prepare'); return; }
        const { filename, content } = preparedDownload;
        const ok = downloadOrOpenMd(filename, content);
        if (ok) {
            setStatus(`✅ Mentve: ${filename}`);
            preparedDownload = null;
            setButtonMode(btn, 'prepare');
        } else {
            setStatus('⚠️ A mentés nem indult el – koppints újra', true);
        }
    }
    ...
    function ensureOverlay() {
        ...
        badge.textContent = `GC→MD v${VERSION} · FF`;

        const btn = document.createElement('button');
        btn.id = BTN_ID;
        btn.dataset.mode = 'prepare';
        btn.textContent = '📥 Markdown letöltése';

        const status = document.createElement('span');
        status.id = STATUS_ID;
        status.textContent = 'Kész – koppints a letöltéshez';
        ...
        // Firefox-fix – kétlépcsős letöltés:
        //  1) koppintás → oldal beolvasása + MD összeállítása (hosszú async: a user-activation lejár)
        //  2) koppintás a zöld „Mentés” gombra → friss user-activation → a letöltés AZONNAL elindul
        //     (nem kell egeret mozgatni / a lapra kattintani, hogy a Firefox tényleg letöltsön)
    ...
    async function runExport(btn) {
        ...
        try {
            ...
            // Firefox-fix: NEM hívunk itt azonnal a.click()-et. A hosszú async beolvasás alatt
            // lejár az eredeti koppintás user-activationje, ezért a Firefox csak a következő
            // valódi interakcióra (pl. egérmozgás) indítaná a letöltést. Helyette a kész MD-t
            // eltároljuk, és a gombot „Mentés” módba tesszük – a következő koppintás friss
            // aktivációval AZONNAL letölt.
            preparedDownload = { filename, content: md };
            setButtonMode(btn, 'save');
            setStatus(`✅ Kész – koppints a „Mentés” gombra: ${filename}`);
        } catch (err) {
